...
public class view_visualizar_acervoCartazes {
    private JPanel panelVisualizar;
    private JTable tableAcervo;
    private JButton btnExcluir;
    private DefaultTableModel tableModel;

    public view_visualizar_acervoCartazes() {
        JFrame frame = new JFrame("Visualizar Acervo Cartazes");
    ...
    private void initializeTable() {
        tableModel = new DefaultTableModel(new Object[]{"ID", "Título", "Ano", "Autor", "Emprestado", "CDU", "Palavras-chave", "Tipo"}, 0);
    ...
    private void loadTableData() {
        try {
            ...
            dao_acervoCartazes.getAllAcervoCartazesDB(acervos);
            for (Acervo acervo : acervos) {
                if (acervo instanceof acervoCartazes) {
                    acervoCartazes cartaz = (acervoCartazes) acervo;
                    tableModel.addRow(new Object[]{
                            cartaz.getAcervoId(),
                            cartaz.getAcervoTitulo(),
                            cartaz.getAcervoAno(),
                            cartaz.getAcervoAutores(),
                            cartaz.isAcervoFlagEmprestado(),
                            cartaz.getAcervoCDU(),
                            cartaz.getAcervoPalavrasChave(),
                            cartaz.getCartazesTipo().getDescricao()
                    });
                }
            }
    ...
    private void deleteSelectedRow() {
        ...
        if (selectedRow >= 0) {
            ...
            acervoCartazes acervoAux = new acervoCartazes();
            acervoAux.setAcervoId(acervoId);
            try {
                if (dao_acervoCartazes.excluirAcervoCartazesDB(acervoAux)) {
