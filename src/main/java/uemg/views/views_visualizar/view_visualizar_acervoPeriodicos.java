...
public class view_visualizar_acervoPeriodicos {
    private JPanel panelVisualizar;
    private JTable tableAcervo;
    private JButton btnExcluir;
    private DefaultTableModel tableModel;

    public view_visualizar_acervoPeriodicos() {
        JFrame frame = new JFrame("Visualizar Acervo Periodicos");
    ...
    private void initializeTable() {
        tableModel = new DefaultTableModel(new Object[]{"ID", "Título", "Ano", "Autor", "Emprestado", "CDU", "Palavras-chave", "Qtd Páginas", "ISBN", "Cidade", "Editora", "Edicao", "Tipo"}, 0);
    ...
    private void loadTableData() {
        try {
            ...
            dao_acervoPeriodicos.getAllAcervoPeriodicos(acervos);
            for (Acervo acervo : acervos) {
                if (acervo instanceof acervoPeriodicos) {
                    acervoPeriodicos periodico = (acervoPeriodicos) acervo;
                    tableModel.addRow(new Object[]{
                            periodico.getAcervoId(),
                            periodico.getAcervoTitulo(),
                            periodico.getAcervoAno(),
                            periodico.getAcervoAutores(),
                            periodico.isAcervoFlagEmprestado(),
                            periodico.getAcervoCDU(),
                            periodico.getAcervoPalavrasChave(),
                            periodico.getPeriodicosQtdPaginas(),
                            periodico.getPeriodicosISBN(),
                            periodico.getPeriodicosCidade(),
                            periodico.getPeriodicosEditora(),
                            periodico.getPeriodicosEdicao(),
                            periodico.getPeriodicosTipo().getDescricao()
                    });
                }
            }
    ...
    private void deleteSelectedRow() {
        ...
        if (selectedRow >= 0) {
            ...
            acervoPeriodicos acervoAux = new acervoPeriodicos();
            acervoAux.setAcervoId(acervoId);
            try {
                if (dao_acervoPeriodicos.excluirAcervoPeriodicosDB(acervoAux)) {
