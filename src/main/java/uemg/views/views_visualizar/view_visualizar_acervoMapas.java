...
public class view_visualizar_acervoMapas {
    private JPanel panelVisualizar;
    private JTable tableAcervo;
    private JButton btnExcluir;
    private DefaultTableModel tableModel;

    public view_visualizar_acervoMapas() {
        JFrame frame = new JFrame("Visualizar Acervo Mapas");
    ...
    private void initializeTable() {
        tableModel = new DefaultTableModel(new Object[]{"ID", "Título", "Ano", "Autor", "Emprestado", "CDU", "Palavras-chave", "Cidade", "Edicao"}, 0);
    ...
    private void loadTableData() {
        try {
            ...
            dao_acervoMapas.getAllAcervoMapasDB(acervos);
            for (Acervo acervo : acervos) {
                if (acervo instanceof acervoMapas) {
                    acervoMapas mapa = (acervoMapas) acervo;
                    tableModel.addRow(new Object[]{
                            mapa.getAcervoId(),
                            mapa.getAcervoTitulo(),
                            mapa.getAcervoAno(),
                            mapa.getAcervoAutores(),
                            mapa.isAcervoFlagEmprestado(),
                            mapa.getAcervoCDU(),
                            mapa.getAcervoPalavrasChave(),
                            mapa.getMapalocal(),
                            mapa.getMapaEdicao()
                    });
                }
    ...
    private void deleteSelectedRow() {
        ...
        if (selectedRow >= 0) {
            ...
            acervoMapas acervoAux = new acervoMapas();
            acervoAux.setAcervoId(acervoId);
            try {
                if (dao_acervoMapas.excluirAcervoMapasDB(acervoAux)) {
